...
const mcpReadiness = [
  ...
  { item: '多人共用', desc: '不是單次 demo，而是多個任務、多人、跨 client 都會用。' },
  ...
  return (
    <PageLayout partIndex={12}>
      ...
      <p className="text-slate-400 leading-relaxed mb-8">
        ...
        它不是每個 demo 的第一步。當流程還在摸索，CLI / export / API script 更快；
      ...
      <div className="rounded-xl border border-white/10 bg-white/[0.02] overflow-hidden mb-5">
        {[
          ...
          <div key={step} className="grid grid-cols-1 gap-2 border-b border-white/5 px-4 py-3 text-sm last:border-0 md:grid-cols-[8rem_1fr]">
            <span className="font-medium text-violet-300">{step}</span>
            <span className="leading-relaxed text-slate-400">{desc}</span>
          </div>
        ))}
      </div>
