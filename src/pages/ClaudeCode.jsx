...
- 送出 commit 前請先確認有沒有遺留 console.log`}
        ...
        {/* ── DEEP DIVE ── */}
        <div className="rounded-2xl border border-slate-500/20 bg-slate-500/5 p-6 mb-8 mt-4">
          ...
          <div className="mb-8">
            <h4 className="text-white font-semibold mb-3 text-sm flex items-center gap-2">
              <span className="w-5 h-5 rounded-full bg-sky-500/30 text-sky-300 text-xs flex items-center justify-center font-bold">1</span>
              多輪對話技術上是怎麼進行的
            </h4>
            <p className="text-slate-400 text-sm leading-relaxed mb-4">
              很多人以為模型有「記憶」。實際上完全沒有——模型本身是無狀態的。
              每一輪對話，<span className="text-white font-medium">你的 client 都必須把完整的對話歷史重新打包傳給 API</span>，
              模型從頭讀一次才能「知道」之前說了什麼。
            </p>
            <div className="rounded-xl bg-black/40 border border-white/10 p-5 font-mono text-xs leading-relaxed mb-4">
              <div className="text-slate-500 mb-2">{/* 第 1 輪 */}{'// 第 1 輪 API 請求'}</div>
              ...
              <div className="text-sky-300">]</div>
              <div className="text-slate-500 mt-3 mb-2">{'// 第 N 輪：歷史 × N 條訊息全部重送，input token 持續累積'}</div>
            </div>
            <p className="text-slate-400 text-sm leading-relaxed">
              這意味著對話越長，<span className="text-amber-300 font-medium">每一輪的 input token 就越多</span>。
          ...
          <div className="mb-8">
            <h4 className="text-white font-semibold mb-3 text-sm flex items-center gap-2">
              <span className="w-5 h-5 rounded-full bg-amber-500/30 text-amber-300 text-xs flex items-center justify-center font-bold">2</span>
              ...
              {[
                {
                  ...
                  color: 'text-sky-300',
                  dot: 'bg-sky-400',
                  content: '你送給模型的所有內容：system prompt、完整對話歷史、工具定義、當前訊息',
                  rate: '1×',
                ...
                },
                {
                  ...
                  color: 'text-rose-300',
                  dot: 'bg-rose-400',
                  content: '開啟 Extended Thinking 時，模型的內部推理過程（你看不到全部，但要付錢）',
                  rate: '5×',
                  rateColor: 'text-amber-400',
                },
              ].map(({ type, color, dot, content, rate, rateColor }) => (
            ...
            <div className="rounded-xl bg-black/40 border border-white/10 p-4 text-xs font-mono">
              <div className="text-slate-500 mb-2">{'// Claude Code 一次典型的「修 bug」請求的 token 分佈'}</div>
              <div className="flex gap-6 flex-wrap">
                <div>
                  <span className="text-sky-300">input:</span>
                  <span className="text-slate-300 ml-2">~8,000 tokens</span>
                  <span className="text-slate-600 ml-2">(歷史 + 相關程式碼)</span>
                </div>
                <div>
                  <span className="text-violet-300">output:</span>
                  <span className="text-slate-300 ml-2">~500 tokens</span>
                </div>
                <div>
                  <span className="text-rose-300">thinking:</span>
                  <span className="text-slate-300 ml-2">~2,000 tokens</span>
                  <span className="text-slate-600 ml-2">(若開啟)</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            <h4 className="text-white font-semibold mb-3 text-sm flex items-center gap-2">
              <span className="w-5 h-5 rounded-full bg-emerald-500/30 text-emerald-300 text-xs flex items-center justify-center font-bold">3</span>
              為什麼要定時清 Session：1M Context 的陷阱
            </h4>
            <p className="text-slate-400 text-sm leading-relaxed mb-4">
              Claude 支援到 200K token 的 context window（部分模式更高），但大不代表好用。有兩個根本問題：
            </p>
            <div className="space-y-3 mb-4">
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4">
                <div className="text-amber-300 font-semibold text-sm mb-2">問題一：成本線性（甚至超線性）增長</div>
                <p className="text-slate-400 text-sm leading-relaxed">
                  每輪對話的 input token = 累積歷史總量。對話進行 10 輪、每輪輸出 1,000 token，
                  到第 10 輪你光是 input 就要付 10,000 token 的費用——而這只是歷史，還沒算當前輸入。
                  任務越複雜、工具呼叫越多，歷史膨脹越快。
                </p>
              </div>
              <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-4">
                <div className="text-rose-300 font-semibold text-sm mb-2">問題二：Attention 機制的注意力稀釋</div>
                <p className="text-slate-400 text-sm leading-relaxed mb-2">
                  Transformer 的 Self-Attention 讓每個 token 都要跟 context 裡的所有其他 token 計算相關性。
                  Context 越長，注意力就越分散——<span className="text-white font-medium">模型對早期內容的「記憶」品質會顯著下降</span>，
                  出現「Lost in the Middle」現象：頭尾記得清楚，中間的重要資訊容易被忽略。
                </p>
                <div className="rounded-lg bg-black/40 border border-white/5 p-3 font-mono text-xs">
                  ...
                  <div><span className="text-rose-300">O(n²)</span><span className="text-slate-400">  ← context 長度 n 的平方</span></div>
                  <div className="text-slate-500 mt-1">{'// 1K tokens → 1M 次計算'}</div>
                  <div className="text-slate-500">{'// 10K tokens → 100M 次計算'}</div>
                  <div className="text-slate-500">{'// 100K tokens → 10B 次計算  ← 費用與延遲爆炸'}</div>
                </div>
              </div>
            </div>
            <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4">
              ...
              <div className="space-y-1.5 text-sm text-slate-300">
                {[
                  ...
                  '用 /cost 監控費用，超過 $0.1 就要評估是否該清掉重來',
                  '把任務結果（如 git diff、測試報告）存到檔案，而不是留在對話裡',
                ].map((t, i) => (
