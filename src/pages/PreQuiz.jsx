...
const questions = [
  {
    ...
    answer: '不是。它們是 coding agent，可以在授權範圍內讀 repo、改檔、跑指令，並把結果交給你 review。這堂課把 Claude Code 與 Codex 都當成 CLI 工具來操作：Claude Code 用 claude，Codex 用 codex；desktop、IDE extension、cloud 只作入口補充。',
  },
  {
    id: 2,
    label: '觀念題',
  ...
  },
  {
    ...
    q: '課前要確認哪兩套 CLI？',
    answer: 'Claude Code 與 Codex 都要確認帳號、repo 權限、外部系統權限，以及允許的 approval / permission 模式。本機先用 claude --version / codex --version 確認 CLI 可用；desktop、IDE extension、cloud 不作為課堂主線。',
  },
  {
    id: 5,
    label: '觀念題',
    labelColor: 'bg-sky-500/20 text-sky-300',
    q: 'Tool call 是不是對話外的免費動作？',
    answer: '不是。Tool call 本身是 assistant message，tool result 也會被接回對話歷史。下一輪模型呼叫會把這些內容一起讀進 context，所以讀檔、grep、跑測試、看 diff 都會影響後續 input token。',
  },
  {
    id: 6,
    label: '操作題',
